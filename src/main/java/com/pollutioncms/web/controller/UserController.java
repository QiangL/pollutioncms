...
import com.pollutioncms.web.vo.UserVO;
...
public class UserController {
    ...
    public Response<?> addUser(@RequestBody @Validated UserVO userVO, BindingResult result) {
        if (result.hasErrors()) {
            return BindErrorHandler.handler(result.getAllErrors());
        }
        logger.info("add user,vo:{}",userVO);
        userService.saveUser(userVO.toDTO());
        return Response.succResp();
    }

    @PostMapping("/deleteUser.mvc")
    @RequiresPermissions("user:delete")
    public Response<?> deleteUser(@RequestBody @Validated(value = {Default.class, UserDTOValidator.NeedId.class}) UserVO userVO,
                             BindingResult result) {

        if (result.hasErrors()) {
            return BindErrorHandler.handler(result.getAllErrors());
        }
        logger.info("delete user,vo:{}", userVO);
        userService.deleteUser(userVO.toDTO());
        return Response.succResp();
    }

    @PostMapping("/updateUser.mvc")
    @RequiresPermissions("user:update")
    public Response<?> updateUser(@RequestBody @Validated(value = {Default.class, UserDTOValidator.NeedId.class}) UserVO userVO,
                             BindingResult result) {

        if (result.hasErrors()) {
            return BindErrorHandler.handler(result.getAllErrors());
        }
        logger.info("update user,vo:{}",userVO);
        userService.updateUserSelective(userVO.toDTO());
        return Response.succResp();
    }
